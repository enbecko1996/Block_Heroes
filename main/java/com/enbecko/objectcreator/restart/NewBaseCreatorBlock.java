...
public abstract class NewBaseCreatorBlock extends BlockContainer{

    protected NewBaseCreatorBlock(Material materialIn) {
        super(materialIn);
    }
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, ItemStack heldItem, EnumFacing side, float hitX, float hitY, float hitZ) {
        TENewBase creatorBlock = (TENewBase) worldIn.getTileEntity(pos);
        System.out.println(creatorBlock+" is Now active: "+creatorBlock.setActive(!creatorBlock.isActive()));
        return true;
    }

    public boolean removedByPlayer(IBlockState state, World world, BlockPos pos, EntityPlayer player, boolean willHarvest) {
        TENewBase creatorBlock = (TENewBase) world.getTileEntity(pos);
