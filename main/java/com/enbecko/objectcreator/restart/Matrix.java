...
public class Matrix {
    ...
    private static final double threshold = 0.0000001F;
    /**
    ...
    public static void multiplyMM(double[] result, int resultOffset,
        ...
        for (int k = 0; k < 16; k++) {
            double sum = 0;
            for (int l = 0; l < 4; l++) {
                sum += lhs[l * 4 + k % 4 + lhsOffset] * rhs[(k / 4) * 4 + l + rhsOffset];
            }
            if(Math.abs(sum - Math.round(sum)) < threshold) {
                result[k + resultOffset] = Math.round(sum);
            } else {
                result[k + resultOffset] = sum;
            }
        }
    }
     ...
     *
     * @param resultVec The double array that holds the result vector.r
    ...
    public static void multiplyMV(double[] resultVec,
        ...
        for (int k = 0; k < 4; k++) {
            double sum = 0;
            for (int l = 0; l < 4; l++) {
                sum += lhsMat[l * 4 + k % 4 + lhsMatOffset] * rhsVec[l + rhsVecOffset];
            }
            if(sum < threshold) {
                resultVec[k + resultVecOffset] = 0;
            } else {
                resultVec[k + resultVecOffset] = sum;
            }
        }
    }
    ...
    public static void perspectiveM(double[] m, int offset,
        ...
        double f = 1.0f / (double) Math.tan(fovy * (Math.PI / 360.0));
    ...
    public static void setRotateEulerM(double[] rm, int rmOffset,
                                       double x, double y, double z) {
        x *= (double) (Math.PI / 180.0f);
        y *= (double) (Math.PI / 180.0f);
        z *= (double) (Math.PI / 180.0f);
